...
describe('Create templates from parameters', () => {
  ...
  it('Should compile orderConfirmation.hbs template and append updateCardUrl if subscription or has subscription id', async () => {
    const order = {
      orderId: 'dummy-order-KYV-1036',
      createdAt: '2021-08-16T07:56:57.599811',
      items: [
        {
          productId: 'productId2',
  ...
  it('Should compile subscriptionPaymentFailed.hbs template to html and assert that it matches snapshot', async () => {
    ...
    const subscription = {
      ...
      renewalDate: '2021-12-13T12:43:28.164Z',
      periodUnit: 'daily',
      periodFrequency: 1,
      periodCount: 2,
      productId: 'productId',
  ...
  it('Should compile subscriptionContract.hbs template to html and assert that it matches snapshot', async () => {
    const order = {
      orderId: 'dummy-order-KYV-466',
      createdAt: '2021-08-16T07:56:57.599811',
      endDate: '2021-09-16T07:56:57.599811',
    ...
    const subscription = {
      ...
      renewalDate: '2023-05-26T12:00:00.000Z',
      startDate: '2023-05-26T12:00:00.000Z',
      endDate: '2023-06-25T23:59:59.999Z',
      billingStartDate: '2023-05-26T12:00:00.000Z',
      periodUnit: 'daily',
      periodFrequency: 1,
      periodCount: 2,
      productId: 'productId',
    ...
    template.setFileName('subscriptionContract')
    const compiledTemplate = template.createTemplate()

    const subscriptionContractTemplate = compiledTemplate(templateParams)
    // Used to make developing faster when html can be "hot reloaded" from file.
    const writeToFile = false
    if (writeToFile) {
      try {
        fs.writeFileSync(
          'packages/message-backend/src/email/create/__snapshots__/subscriptionContract.html',
    ...
    expect(subscriptionContractTemplate).toMatchSnapshot()
  })

  it('Should compile subscriptionCardExpired.hbs template to html and assert that it matches snapshot', async () => {
    const order = {
      orderId: 'dummy-order-KYV-351',
      createdAt: '2021-08-16T07:56:57.599811',
      endDate: '2021-09-16T07:56:57.599811',
      renewalDate: '2021-09-13T07:56:57.599811',
      items: [
        {
          productId: 'productId2',
          productName: 'Pysäköintivyöhyke K',
      ...
      updateCardUrl: `http://localhost:3000/dummy-order-KYV-351/update-card?user=userId`,
    } as Order
    ...
    const subscription = {
      ...
      startDate: '2021-12-13T12:43:28.165Z',
      endDate: '2022-01-01T07:56:57.599811',
      renewalDate: '2022-03-30T07:56:57.599811',
      billingStartDate: '2021-12-13T12:43:28.164Z',
      periodUnit: 'daily',
      periodFrequency: 1,
      periodCount: 2,
    ...
    // TODO Add mocked subscription data -> get from backend with card information
    const templateParams: SubscriptionCardExpiredEmailParameters = {
      order: order,
      subscription: subscription,
    }

    const template = HandleBarTemplate<SubscriptionCardExpiredEmailParameters>(
      'fi'
    )
    template.setFileName('subscriptionCardExpired')
    const compiledTemplate = template.createTemplate()
    ...
    if (writeToFile) {
      try {
        fs.writeFileSync(
          'packages/message-backend/src/email/create/__snapshots__/subscriptionCardExpired.html',
          subscriptionCardExpiredTemplate
        )
        //file written successfully
      } catch (err) {
        console.error(err)
    ...
    expect(subscriptionCardExpiredTemplate).toMatchSnapshot()
  })

  it('Should compile orderConfirmation.hbs template with invoice data to html and assert that it matches snapshot', async () => {
    const order = {
      orderId: 'dummy-invoice-order-KYV-575',
      createdAt: '2021-08-16T07:56:57.599811',
      items: [
        {
          productId: 'productId2',
