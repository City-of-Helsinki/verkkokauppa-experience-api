...
import {
  createPaymentRedirectUrlFromPaymentReturnStatus,
  createUserRedirectUrl,
  isAuthorized,
  isPaid,
} from './paymentReturnService'
...
describe('Test User redirection creation', () => {
  ...
  it('Redirect url creation function test', async () => {
    ...
    const createdUrl = createPaymentRedirectUrlFromPaymentReturnStatus(
      mockVismaStatus,
      orderMock,
      mockRedirectUrl,
      true
    ...
    const createdUrl2 = createPaymentRedirectUrlFromPaymentReturnStatus(
      mockVismaStatus,
      orderMock,
      mockRedirectUrl,
      true
    ...
    const createdUrl3 = createPaymentRedirectUrlFromPaymentReturnStatus(
      mockVismaStatus,
      orderMock,
      mockRedirectUrl,
      true
...
describe('check is functions', () => {
  it('isPaid', () => {
    let mockVismaStatus = {
      paymentPaid: true,
    } as VismaStatus

    let mockPaytrailStatus = {
      paymentPaid: true,
    } as PaytrailStatus

    expect(isPaid(mockVismaStatus)).toEqual(true)
    expect(isPaid(mockPaytrailStatus)).toEqual(true)
  })
  ...
  it('isAuthorized', () => {
    ...
    expect(isAuthorized(mockVismaStatus)).toEqual(true)
    expect(isAuthorized(mockPaytrailStatus)).toEqual(true)

    mockVismaStatus.paymentPaid = true
    mockPaytrailStatus.paymentPaid = true
    expect(isAuthorized(mockVismaStatus)).toEqual(false)
    expect(isAuthorized(mockPaytrailStatus)).toEqual(false)
  })
})
