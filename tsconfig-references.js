...
;(async function () {
  ...
  for (const name in workspaces) {
    ...
    if (fs.existsSync(tsconfigPath)) {
      ...
      workspaceConfig.compilerOptions.paths = {}
      for (const dependency of workspace.workspaceDependencies) {
        ...
        if (fs.existsSync(path.resolve(dependecyLocation, 'tsconfig.json'))) {
          ...
          const dependencyName = workspaces[dependency].location
            .split('/')
            .pop()
          workspaceConfig.compilerOptions.paths[
            `@verkkokauppa/${dependencyName}`
          ] = [path.relative(location, dependecyLocation)]
        }
      }
      fs.writeFileSync(
        tsconfigPath,
